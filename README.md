...
## ビジュアルコンセプト
システムの実装イメージおよびハードウェアのプロトタイプ設計です。

### システム・モニタリング画面 (Web UI)
![System Monitoring UI](docs/images/screen-mockup.jpeg)
*VercelでホストされたNext.js PWAによる監視インターフェース。パニック時でも直感的に「1秒で状況把握、0.5秒で停止」ができるデザインを採用しています。*

### Cocoba ロボット本体 (Hardware Prototype)
![Hardware Prototype](docs/images/device-mockup.png)
*ルンバをベースとした移動機構に、ドーム（シールド）を投下するためのカスタムアームを搭載。AI追跡用のArUcoマーカーを備え、自律的な排泄物隔離を実現します。*

### 背景と目的
老犬のここちゃんにとって、新しいトイレトレーニングは困難です。飼い主が不在の際、排泄物を踏んで床全体が汚れてしまう事故が発生しており、帰宅後の清掃負担とここちゃんへの衛生面での懸念が課題となっていました。
